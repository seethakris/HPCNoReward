    ...
    for k in range(niter):
        ...
        for di in np.arange(1, npix - 2):
            ...
            if Cmax > 0:
                ...
                if flip == 2:
                    isort = np.hstack(
                        (np.arange(xstart), np.arange(xstart, xstart + di + 1)[::-1], np.arange(xstart + di + 1, npix)))
                elif flip == 1:
...
def create_ND_basis(dims, nclust, K, flag=True):
    ...
    if dims == 1:
        ...
        for k in range(K):
            if flag:
                S[k, :] = np.sin(
                    math.pi + (k + 1) % 2 * math.pi / 2 + 2 * math.pi / nclust * (xs + 0.5) * int((1 + k) / 2))
            else:
...
class Rastermap:
    ...
    def _map(self, X, dims, nclust, xid, SALL=None):
        ...
        if self.annealing:
            ...
            ncomps_anneal = np.concatenate(
                (ncomps_anneal[:10], ncomps_anneal[2:10], ncomps_anneal[4:], SALL.shape[0] * np.ones(20)),
                axis=0).astype('int')
        else:
