...
def run_svm_on_task(classifier_type, xdata, ydata, tracklength, trackbins, task, plotpdf):
    ...
    SVMmodel = SVM.fit_SVM(x_train=taskmodel['x_train'], y_train=taskmodel['y_train'],
                                        classifier_type=classifier_type)
    taskmodel['scores'], taskmodel['y_pred'], taskmodel['y_prob'] = SVM.validate_model(classifier_type=classifier_type,
                                                                                       model=SVMmodel,
...
class SVM(object):
    ...
    def fit_SVM(x_train, y_train, classifier_type='Bayes'):
        ...
        if classifier_type == 'SVM':
            ...
            model = Sequential()
            epochs = 20
            units = [np.size(x_train, 1), 500]
            batch_size = 20
            # Add a single Dense NN layer
            model.add(Dense(units[0], input_dim=np.size(x_train, 1), activation='relu'))
            model.add(Dense(np.size(y_cat, 1)))  # output layer
            model.compile(loss='mse', optimizer='adam', metrics=['accuracy'])  # Compile model

            # Fit model
            model.fit(x_train, y_cat, epochs=epochs, batch_size=batch_size,
                      verbose=0)
    ...
    @staticmethod
    def validate_model(model, x_test, y_test, task, plotflag, classifier_type='Bayes', **kwargs):
        ...
        if plotflag:
            ...
            axis.plot(y_pred, '|', linewidth=1, color='lightblue', markersize=10, label='Predicted position')
    ...
    def k_foldvalidation(self, x, y, task, split_size, classifier_type):
        ...
        for train_index, test_index in k.split(x):
            ...
            ycv_scores, ycv_predict, ycv_probability = self.validate_model(classifier_type=classifier_type,
        ...
        for n, ns in enumerate(numsamples):
            ...
            for i in np.arange(iterations):
                ...
                for train_index, test_index in k.split(x_resample):
                    ...
                    scores, prediction, probability = self.validate_model(classifier_type=classifier_type,
...
class SVMValidationPlots(object):
    ...
    @staticmethod
    def plot_decoderaccuracy_with_numcells(axis, modeldataframe, taskdict, taskcolors):
