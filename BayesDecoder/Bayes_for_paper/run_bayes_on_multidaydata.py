...
def runSVM(animalinfo):
    ...
    data = LoadData(FolderName=animalinfo['foldername'], Task_NumFrames=animalinfo['task_numframes'],
                    TaskDict=animalinfo['task_dict'], NoRewardTasks=animalinfo['NoRewardTasks'],
                    framestokeep=animalinfo['task_framestokeep'], v73_flag=animalinfo['v73_flag'])
    ...
    for keys in animalinfo['task_dict'].keys():
        if keys in animalinfo['NoRewardTasks']:
            Xdata[keys] = data.Fc3data_dict[keys][:, data.good_running_index[keys]].T[
                          data.lickstopframe[keys]:animalinfo['task_framestokeep'][keys], :]
            Ydata[keys] = bin_behdata.position_binary[keys][
                          data.lickstopframe[keys]:animalinfo['task_framestokeep'][keys]]

            Xdata[f'%sall' % keys] = data.Fc3data_dict[keys][:, data.good_running_index[keys]].T[
                                     :animalinfo['task_framestokeep'][keys], :]
            Ydata[f'%sall' % keys] = bin_behdata.position_binary[keys][
                                     :animalinfo['task_framestokeep'][keys]]
        else:
            Xdata[keys] = data.Fc3data_dict[keys][:, data.good_running_index[keys]].T[
                          :animalinfo['task_framestokeep'][keys], :]
    ...
    for t in animalinfo['task_dict'].keys():
        taskmodel[t] = run_svm_on_task(xdata=Xdata[t], ydata=Ydata[t], tracklength=animalinfo['tracklength'],
...
def run_svm_on_task(xdata, ydata, tracklength, trackbins, task, plotpdf):
    taskmodel = OrderedDict()
...
class LoadData(object):
    def __init__(self, FolderName, Task_NumFrames, TaskDict, NoRewardTasks, framestokeep, v73_flag):
        print('Loading Data')
        ...
        self.load_lapparams()
        self.lickstopframe = {keys: [] for keys in self.NoRewardTasks}
        for i in self.NoRewardTasks:
            self.lickstopframe[i] = np.where(self.good_lapframes[i] == self.lickstoplap[i])[0][0]
...
class SVM(object):
    ...
    @staticmethod
    def fit_SVM(x_train, y_train, classifiertype='Bayes'):
    ...
    @staticmethod
    def validate_model(model, x_test, y_test, task, plotflag, **kwargs):
        ...
        if plotflag:
            fs, axis = plt.subplots(1, figsize=(10, 2))
            img = axis.imshow(y_prob.T, cmap='Blues', aspect='auto', vmin=0, vmax=0.5, origin='lower')
            axis.set_title(task)
            axis.set_ylabel('Binned Position')
            axis.set_xlabel('Frames')
            CommonFunctions.create_colorbar(fighandle=fs, axis=axis, imghandle=img, title='Probability')

            # ax2 = axis.twinx()
            axis.plot(y_pred, linewidth=2, color='lightblue', label='Predicted position')
            axis.plot(y_test, linewidth=1, color='k')

            if 'plotpdf' in kwargs.keys():
                kwargs['plotpdf'].savefig(fs, bbox_inches='tight')

        return scores, y_pred, y_prob

    def k_foldvalidation(self, x, y, task, split_size):
        ...
        nbcv_dataframe = pd.DataFrame(
            columns=['CVIndex', 'ModelAccuracy', 'R2', 'rho', 'Y_diff', 'mean_errorprob', 'sem_errorprob', 'y_test',
                     'y_predict'])
        for train_index, test_index in k.split(x):
            ...
            # Run Model
            cvsnbmodel = self.fit_SVM(x_train=xcv_train, y_train=ycv_train)

            # evaluate the model and save evalautions
            ycv_scores, ycv_predict, ycv_probability = self.validate_model(model=cvsnbmodel, x_test=xcv_test,
        ...
        for n, ns in enumerate(numsamples):
            ...
            for i in np.arange(iterations):
                ...
                for train_index, test_index in k.split(x_resample):
                    ...
                    nbpfmodel = self.fit_SVM(x_rs_train, y_rs_train)
                    scores, prediction, probability = self.validate_model(model=nbpfmodel, x_test=x_rs_test,
...
class SVMValidationPlots(object):
    ...
    @staticmethod
    def plot_decoderaccuracy_with_numcells(axis, modeldataframe, taskdict, tasks_to_plot, taskcolors):
        for n, t in enumerate(tasks_to_plot):
            numcells_df = modeldataframe[t]['Numcells_Dataframe']
            sns.pointplot(x='SampleSize', y='score', data=numcells_df, color=taskcolors[n], ax=axis)

        axis.set_ylim((0, 0.5))

        axis.legend(handles=axis.lines[::len(numcells_df['SampleSize'].unique()) + 1], labels=tasks_to_plot,
                    loc='upper center', bbox_to_anchor=(0.5, -0.3), ncol=4)
        axis.set_xlabel('Percentage of active cells used')
        axis.set_ylabel('Decoding accuracy')
        axis.set_aspect(aspect=8)
        plt.close(2)
        pf.set_axes_style(axis, numticks=4)

    @staticmethod
    def plot_decoderaccuracy_with_tracklength(axis, modeldataframe, tracklength, trackbins, tasks_to_plot, taskcolors):
        ...
        for n, t in enumerate(tasks_to_plot):
            ...
            meandiff = np.mean(Y_diff_by_track_mean, 0)
            semdiff = scipy.stats.sem(Y_diff_by_track_mean, 0)
            error1, error2 = meandiff - semdiff, meandiff + semdiff
            axis.plot(np.arange(numbins), meandiff, color=taskcolors[n])
            axis.fill_between(np.arange(numbins), error1, error2, alpha=0.5, color=taskcolors[n])
            axis.set_xlabel('Track Length (cm)')
            axis.set_ylabel('Difference in decoder accuracy (cm)')
...
class CommonFunctions(object):
    ...
    @staticmethod
    def create_colorbar(fighandle, axis, imghandle, title):
        ...
        cb = fighandle.colorbar(imghandle, cax=axins, pad=0.2, ticks=[0, 0.5])
        cb.set_label(title, rotation=270, labelpad=12)
        cb.ax.tick_params(size=0)


class runreferencemodel(object):
    def __init__(self, animalinfo, savedmodel, taskcolors, reftask='Task1', task_to_fit='Task1', norewardtasks='Task1'):
        self.savedmodel = savedmodel
        self.reftask = reftask
        self.task_to_fit = task_to_fit
        self.norewardtasks = norewardtasks

        decoderfit = self.fit_classifier_withreference()
        data = LoadData(FolderName=animalinfo['foldername'], Task_NumFrames=animalinfo['task_numframes'],
                        TaskDict=animalinfo['task_dict'], NoRewardTasks=animalinfo['NoRewardTasks'],
                        framestokeep=animalinfo['task_framestokeep'], v73_flag=animalinfo['v73_flag'])

        fs, ax = plt.subplots(len(task_to_fit), sharex='all', sharey='all')
        for n, t in enumerate(task_to_fit):
            self.PlotLapwiseAccuracy(axis=ax[n],
    ...
    def fit_classifier_withreference(self, classifiertype='Bayes'):
        taskfit = OrderedDict()
        # Run model on all of control data
        xtrain = self.savedmodel['Xdata'][self.reftask]
        ytrain = self.savedmodel['Ydata'][self.reftask]
        referencemodel = SVM.fit_SVM(xtrain, ytrain, classifiertype='Bayes')

        for t in self.task_to_fit:
            refclassifierfit = OrderedDict()
            if t in self.norewardtasks:  # If task has no reward then use the all version of dataset
                refclassifierfit['xtest'] = self.savedmodel['Xdata'][f'%sall' % t]
                refclassifierfit['ytest'] = self.savedmodel['Ydata'][f'%sall' % t]
            else:
                refclassifierfit['xtest'] = self.savedmodel['Xdata'][t]
                refclassifierfit['ytest'] = self.savedmodel['Ydata'][t]

            refclassifierfit['score'], refclassifierfit['ypred'], refclassifierfit['yprob'] = SVM.validate_model(
                referencemodel, x_test=refclassifierfit['xtest'], y_test=refclassifierfit['ytest'],
                task=f'Apply %s model on %s' % (self.reftask, t),
                plotflag=1)
    ...
    @staticmethod
    def PlotLapwiseAccuracy(axis, lapframes, Y_actual, Y_predicted, numlaps, licks, color, trackbins):
        average_lap_diff = []
        standard_error_lap_diff = []
        for l in np.arange(numlaps - 1):
            laps = np.where(lapframes == l + 1)[0]
            average_lap_diff.append(np.mean(np.abs(Y_actual[laps] - Y_predicted[laps])) * trackbins)
            standard_error_lap_diff.append(scipy.stats.sem(np.abs(Y_actual[laps] - Y_predicted[laps])) * trackbins)
        ...
        axis.plot(np.arange(numlaps - 1), average_lap_diff, linewidth=2, color=color)
        axis.fill_between(np.arange(numlaps - 1), error1, error2, alpha=0.5, color=color)

        axis.set_ylabel('Prediction\nError (cm)')
        for l in np.arange(numlaps - 1):
            if licks[l] > 0:
                axis.axvline(l, color='grey', alpha=0.5, linewidth=2)

        axis.set_aspect(aspect=0.2)
