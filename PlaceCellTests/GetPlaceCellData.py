...
class GetData:
    ...
    def calculate_pfparameters(self):
        ...
        self.pfparams_df = pd.DataFrame(
            columns=['Task', 'CellNumber', 'PlaceCellNumber', 'NumPlacecells', 'COM', 'WeightedCOM', 'Precision',
                     'Width', 'FiringRatio', 'Firingintensity', 'Stability'])
        for t in self.PlaceFieldData:
            ...
            for n in np.arange(np.size(self.sig_PFs_cellnum_revised[taskname])):
                ...
                for p1 in np.arange(beg, self.numPFs_incells_revised[taskname][n]):
                    ...
                    # plt.imshow(data_bw.T)
                    # plt.title(f'actual %0.2f corrected %0.2f' % (
                    # np.nanmean(stability), numlaps_withfiring * np.nanmean(stability)))
                    # plt.show()
                    # ut = np.triu(stability, k=1)
                    # ut[ut == 0] = np.nan
                    # plt.imshow(ut, aspect='auto', vmin=0, vmax=1)
                    # plt.title(np.nanmean(ut))
                    # plt.show()
                    # plt.title(f'Cell %d COM %0.2f, precision %0.2f' % (
                    #     self.sig_PFs_cellnum[taskname][n], np.mean(weighted_com), precision))
                    # plt.show()
    ...
    def plot_reward_data_per_cell(self, reward_data):
        pdf = PdfPages(os.path.join(self.SaveFolder, 'Rewardzonefiring.pdf'))
        for c in np.arange(self.numcells):
